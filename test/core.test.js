...
test('project round-trips through serialize → load (geometry, style, columns, layers, params)', () => {
  ...
  ps.toggleLayer('poles'); ps.toggleLayer('contours'); ps.setParams({ cLevels: 6, cMethod: 'kamb', cRamp: 'magma', tableOpen: true, tablePos: [0.2, 0.7], tableColW: [90, 120] });
  a.add(new PoleSet({
  ...
  a.setPageShow(true); a.setPageAspect('a4-landscape'); a.setFigureBg('paper'); a.setExportDpi(300);
  a.setViewport({ tx: 12, ty: -8, scale: 1.5 }); a.setRotation([1, 0, 0, 0, 1, 0, 0, 0, 1]);
  ...
  assert.deepEqual([b.pageShow(), b.pageAspect(), b.figureBg(), b.exportDpi()], [true, 'a4-landscape', 'paper', 300]);   // figure/page config persists
  assert.deepEqual(b.viewport(), { tx: 12, ty: -8, scale: 1.5 });   // pan/zoom view state persists
  assert.deepEqual(b.rotation(), [1, 0, 0, 0, 1, 0, 0, 0, 1]);       // orientation persists
  ...
  assert.equal(bed.currentParams().tableOpen, true);                  // floating-table panel state persists
  assert.deepEqual(bed.currentParams().tablePos, [0.2, 0.7]);
  assert.deepEqual(bed.currentParams().tableColW, [90, 120]);          // resized column widths persist
